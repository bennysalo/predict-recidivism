...
```{r}
plot_comprison <- function(model_list, nr, upper_xlim = 0.3, 
...
```{r}
plot_comprison(trained_mods_glmnet, 1)
glmnet_grid$alpha2[[1]] <- c(0.5, 0.6, 0.7, 0.8, 0.9, 1.0)
```
```{r}
plot_comprison(trained_mods_glmnet, 2)
glmnet_grid$alpha2[[2]] <- c(0, 0.1, 0.2)
```
```{r}
plot_comprison(trained_mods_glmnet, 3, upper_xlim = 1)
glmnet_grid$alpha2[[3]]  <- c(0.1, 0.2, 0.3)
```

```{r}
plot_comprison(trained_mods_glmnet, 4, upper_xlim = 0.8, lower_ylim_range = 0.03)
glmnet_grid$alpha2[[4]]  <- c(0, 0.1)
```
```{r}
plot_comprison(trained_mods_glmnet, 5)
glmnet_grid$alpha2[[5]]  <- c(0.7, 0.8, 0.9, 1)
```


```{r}
plot_comprison(trained_mods_glmnet, 6, upper_xlim = 0.8)
glmnet_grid$alpha2[[6]]  <- c(0, 0.1)
```
```{r}
plot_comprison(trained_mods_glmnet, 7)
glmnet_grid$alpha2[[7]]  <- c(0.7, 0.8, 0.9, 1)
```

```{r}
plot_comprison(trained_mods_glmnet, 8, upper_xlim = 0.8)
