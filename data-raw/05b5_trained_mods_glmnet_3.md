...
trained_mods_glmnet_3_LL <-      # new name for new set of results
  purrr::map2(
...
```
    ...
    ## Time difference of 2.352397 mins

``` r
start <- Sys.time()

trained_mods_glmnet_3_AUC <-      # new name for new set of results
  purrr::map2(
    .x = glmnet_grid$formula,
    .y = glmnet_grid$best_for_AUC,  # updated
    .f = ~ caret::train(
      form      = .x,
      data      = training_set,
      method    = "glmnet",
...
```
    ...
    ## Time difference of 2.35192 mins

Name models

``` r
names(trained_mods_glmnet_3_LL) <- paste0(glmnet_grid$model_name)
names(trained_mods_glmnet_3_AUC) <- paste0(glmnet_grid$model_name, "_AUC")
```
...
``` r
devtools::wd()
saveRDS(trained_mods_glmnet_3_LL, "not_public/trained_mods_glmnet_3_LL.rds")
saveRDS(trained_mods_glmnet_3_AUC, "not_public/trained_mods_glmnet_3_AUC.rds")
```
...
```
    ...
    ## [53] kernlab_0.9-27          glmnet_2.0-16          
