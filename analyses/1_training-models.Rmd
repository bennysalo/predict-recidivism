...
```{r}
write_arg_list <- function(model_type) {
  if (model_type == "glm") {
    ...
    specific_args <- list(method = "glmnet",
                          family = "binomial",
                          standardize = TRUE,
                          tuneGrid = expand.grid(c(0, 0.25, 0.5, 0.75, 1))
  )
  } else if (model_type == "rf") {
    specific_args <- list(method = "rf",
                          tuneGrid = expand.grid()) # SOLVE
  }
...
test_set     <- FinPrisonMales[ (1:nrow(FinPrisonMales) %in% 
                                   four_folds[[4]]), all_variables]
training_set <- FinPrisonMales[!(1:nrow(FinPrisonMales) %in% 
                                   four_folds[[4]]), all_variables]
