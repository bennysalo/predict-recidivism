...
```{r}
write_arg_list <- function(model_type, n_preds) {
  if (model_type == "glm") {
    specific_args <- list(method = "glm",
                          family = "binomial")
  } else if (model_type == "glmnet") {
    specific_args <- list(method = "glmnet",
                          family = "binomial",
                          standardize = TRUE,
    ...
    specific_args <- list(method = "rf",
                          tuneGrid = expand.grid(mtry = m_try_seq)) # SOLVE
  }
...
model_grid$specific_args <-
  purrr::map2(
    .x = model_grid$model_type,
    .y = model_grid$n_preds,
    .f = ~ write_arg_list(.x, .y)
    )
```
...
test3 <- my_train(outcome = rf_grid$outcome[1], 
                  predictors = rf_grid$predictors[1], 
                  specific_args = rf_grid$specific_args[1])
```
