...
test_set_predictions <- map_df(model_list, .f = ~ predict_test_set(.x))
...
```{r}
devtools::use_data(auc_tbl, overwrite = TRUE)
```
...
# Calculate Cohens' d and r 

```{r}
calculate_logit <- function(prob) {
  log(prob / (1 - prob))
}

test_set_predictions_logit <- test_set_predictions %>% 
  transmute_at(.vars = 3:ncol(test_set_predictions), .f = calculate_logit)
```

```{r}
test_set_predictions %>% select(newO_violent, model_names_V) %>% 
  mutate(newO_violent = as.integer(newO_violent)) %>% 
  cor() %>% 
  as.data.frame() %>% 
  rownames_to_column("model") %>% 
  filter(model %in% best_models_tbl$model)

test_set_predictions %>% select(reoffenceThisTerm, model_names_G) %>% 
  mutate(reoffenceThisTerm = as.integer(reoffenceThisTerm)) %>% 
  cor() %>% 
  as.data.frame() %>% 
  rownames_to_column("model") %>% 
  filter(model %in% best_models_tbl$model)
...
test_set_predictions %>% select(reoffenceThisTerm, model_names_G) %>% 
  ...
  summarise_at(.vars = (-1), .funs = mean) %>% 
  gather(key = "model", value = "M", -1) %>% 
  spread(key = reoffenceThisTerm, value = "M")

G_sds <-
test_set_predictions %>% select(reoffenceThisTerm, model_names_G) %>% 
  group_by(reoffenceThisTerm) %>% 
  summarise_at(.vars = (-1), .funs = sd) %>% 
  gather(key = "model", value = "SD", -1) %>% 
  spread(key = reoffenceThisTerm, value = "SD")

full_join(G_means, G_sds, by = "model", suffix = c("_M", "_SD"))
