...
auc_tbl %>% 
  mutate(predictors = 
           ...
           ifelse(str_detect(model, "staticp"), "RStatic",
...
Predictive Power of RITA-Items
```{r}
auc_tbl %>%  select(1:4) %>% filter(str_detect(model, "dynamic"))
auc_tbl %>%  select(1:4) %>% filter(str_detect(model, "static"))
```
