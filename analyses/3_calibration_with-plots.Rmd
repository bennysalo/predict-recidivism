...
Put each prediction into a bin (1 to 5) based on quintile
```{r}
bins <-
  predictions %>%
...
```{r}
sub_dframe <- data_frame(predictions[1], bins[1], test_set["reoffenceThisTerm"])

sub_dframe %>% 
  group_by(G_staticp_glm.bin) %>% 
    summarise(n = n(), 
              obs = sum(as.numeric(reoffenceThisTerm) == 2),
              prop = obs/n * 100,
              mean_rate = mean(G_staticp_glm)*100) %>% 
    select(-n, -obs)

get_bin_stats <- function(est_probs, quantile, outcome) {
  bin_stats <- 
}
```
