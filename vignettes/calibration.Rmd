...
Calculating expected and observed outcomes by quintile will be facilitated by having a seperate data frame for each model including the estimated probabilites, corresponding quintile, and pertinent outcome.

We start by making a list vectors of estimated probabilites that we then turn to a list of data frames - at this point with a single column.
```{r}
...
For each data frame we caluclate the expected and observed recidivism rates. We get a list of data frames with 5 rows (one per quintile) and three columns (quintile, observed rate and expected rate.)
